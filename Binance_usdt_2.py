...
def fetch_klines_4h(symbol: str, limit=120):
    data = http_get(
        [f"{b}/api/v3/klines" for b in SPOT_BASES],
    ...
    df = pd.DataFrame(
        data,
        columns=["t", "o", "h", "l", "c", "v", "t2", "q", "n", "tb", "tq", "i"],
    )
...
def futures_symbol(symbol_spot: str) -> str | None:
    """En USDT-M el símbolo coincide con spot (BTCUSDT, ETHUSDT, ...)."""
    if not FUTU_BASES:
        return None
    cand = symbol_spot
    ...
    try:
        data = http_get(
            [f"{b}/fapi/v1/exchangeInfo" for b in FUTU_BASES], {"symbol": cand}
        )
...
def fetch_open_interest_hist(symbol_fut: str, period="1h", limit=10) -> pd.Series | None:
    ...
    for b in FUTU_BASES:
        try:
            arr = http_get(
                f"{b}/futures/data/openInterestHist",
                {"symbol": symbol_fut, "period": period, "limit": limit},
            )
            if arr:
                df = pd.DataFrame(arr)
                df["sumOpenInterest"] = pd.to_numeric(
...
def fetch_base_df(logger=log_default) -> pd.DataFrame:
    """Construye DF base (spot USDT en TRADING, cruzado con ticker 24h)."""
...
def build_df(limit_symbols: int | None = None, logger=log_default):
    ...
    logger(
        f"Iniciando job · LIMIT_SYMBOLS={LIMIT} MAX_WORKERS={MAX_WORKERS} BUDGET={TIME_BUDGET}s"
    )

    base = fetch_base_df(logger)
    if base.empty:
        return pd.DataFrame(), {"count": 0}

    base = (
        base.sort_values("quote_volume_24h", ascending=False)
        .head(LIMIT)
    ...
    df = df.sort_values(
        ["GHI", "quote_volume_24h"],
        ascending=[False, False],
        na_position="last",
    ).reset_index(drop=True)

    # CSV temporal (útil para depuración en Render)
    try:
